			...
			while(!done) {
				...
				if(GetAsyncKeyState(VK_ESCAPE) < 0) {
				 ...
				 if(tmpIterations == 199){
				 	if(fabs(prevState.angle) <= 0.05 && (fabs(prevState.angle - tmpPrevAngle) <= 0.03) && fabs(prevState.x) <= 0.02 && fabs(prevState.x - tmpPrevX) <= 0.09){
				 		cout << "\nBalanced!" << endl;
				 		stabilized = true;
				 		done = true;
				 		// getch();
				...
				// Clamp fuzzy input to safe range (prevents Sum2 = 0)
				if (inputs[INPUT_X] >  40.0f) inputs[INPUT_X] =  40.0f;
				if (inputs[INPUT_X] < -40.0f) inputs[INPUT_X] = -40.0f;
				...
				float F_angle = -fuzzy_system(inputs, g_fuzzy_system);
				float F_pos = -(8.0f * prevState.x + 5.0f * prevState.x_dot);  // Reduced from 50/25
				prevState.F = F_angle + F_pos;
...
void saveDataToFile(string fileName){
	...
	if (getcwd(cwd, sizeof(cwd)) != NULL) {
		...
		cout << "Saving file to: " << cwd << "/" << fileName << endl;
		cout << "========================================\n" << endl;
	}
   ...
   if (checkFile.good()) {
       cout << "\n✓ SUCCESS! Data set saved (File: " << fileName << ")" << endl;
       if (getcwd(NULL, 0) != NULL) {
           char cwd[1024];
           getcwd(cwd, sizeof(cwd));
           cout << "✓ File location: " << cwd << "/" << fileName << endl;
       }
       cout << "========================================\n" << endl;
   } else {
       cout << "\n✗ ERROR: Failed to create file!" << endl;
   }
   checkFile.close();
